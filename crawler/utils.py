...
def read_version_payload(stream):
    ...
    r["relay"] = bool(little_endian_to_int(stream.read(1)))
    return r
...
def read_payload(command, stream):
    ...
    return command_key[command](BytesIO(stream))
...
def read_msg(stream):
    ...
    try:
        payload = read_payload(command, payload)
    except(KeyError):
        pass
